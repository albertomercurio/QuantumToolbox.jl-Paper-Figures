...
const a0 = kron(destroy(N0), qeye(2))

const σm0 = kron(qeye(N0), sigmam())
const σz0 = kron(qeye(N0), sigmaz())
...
const a = kron(destroy(N_dsf), qeye(2))
const σm = kron(qeye(N_dsf), sigmam())
const σz = kron(qeye(N_dsf), sigmaz())
...
h = QuantumCumulants.tensor(h1, ha)

# Define the fundamental operators
@qnumbers a₂::Destroy(h,1)
σm₂ = Transition(h,:σ,:g,:e)
σz₂ = σm₂'*σm₂ - σm₂*σm₂'
@cnumbers F₂ Δc₂ Δa₂ U₂ γc₂ γa₂ g₂

# Hamiltonian
H₂ = Δc₂ * a₂'*a₂ + Δa₂/2 * σz₂ + U₂*(a₂^2)'*a₂^2 + g₂*(a₂*σm₂' + a₂'*σm₂) + F₂*(a₂+a₂')
...
snapshot_point1 = posFig(ax1, tlist_save[1], 0)
snapshot_point2 = posFig(ax1, tlist_save[2], 0)
snapshot_point3 = posFig(ax1, tlist_save[3], 0)
...
poly_x = [snapshot_point2[1], ax_origin[1], ax_origin[1] + ax_widths[1]]
poly_y = [snapshot_point2[2], ax_origin[2] + ax_widths[1], ax_origin[2] + ax_widths[2]]
poly!(fig.scene, collect(zip(poly_x, poly_y)); poly_kwargs...)

ax_origin = ax4.scene.viewport[].origin
ax_widths = ax4.scene.viewport[].widths
poly_x = [snapshot_point3[1], ax_origin[1], ax_origin[1] + ax_widths[1]]
